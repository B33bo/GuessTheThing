...
{
    private static List<string> prebuiltGames;
    ...
    private static void LoadPrebuiltGames()
    {
        prebuiltGames = new();
        if (!Directory.Exists("./Prebuilt/"))
            return;
        string[] files = Directory.GetFiles("./Prebuilt/");
        for (int i = 0; i < files.Length; i++)
            prebuiltGames.Add(Path.GetFileNameWithoutExtension(files[i]).ToLower());
    }
    ...
    private static void ListFiles()
    {
        ...
        foreach (var prebuiltGame in prebuiltGames)
            Console.WriteLine(prebuiltGame);
    ...
    private static string GetFilePos(string input)
    {
        ...
        if (prebuiltGames.Contains(input))
            return $"./Prebuilt/{input}.txt";
